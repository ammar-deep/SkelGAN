...
def main():
    ...
    model = create_model(examples.trg_font, examples.trg_skeleton, examples.style_labels, examples.char_labels, args)
    ...
    with tf.name_scope("encode_images"):
        display_fetches = {
            ...
            "trg_font": tf.map_fn(tf.image.encode_png, converted_trg_font, dtype=tf.string, name="tgt_fonts"),
            "trg_skeleton": tf.map_fn(tf.image.encode_png, converted_trg_skeleton, dtype=tf.string, name="tgt_skeletons"),
    ...
    with tf.name_scope("f2s_predict_real_summary"):
        tf.summary.image("f2s_predict_real", tf.image.convert_image_dtype(model.f2s_predict_real, dtype=tf.uint8))
    ...
    tf.summary.scalar("f2s_discriminator_loss_real_char", model.f2s_disc_loss_real_char)
        ...
        if args.mode == "test":
            ...
            for step in range(max_steps):
                ...
                if should(args.progress_freq):
                    ...
                    fetches["f2s_disc_loss_real_char"] = model.f2s_disc_loss_real_char
                ...
                if should(args.progress_freq):
                    ...
                    print("f2s_disc_loss_real_char", results["f2s_disc_loss_real_char"])

...
Examples = collections.namedtuple("Examples", "paths, trg_font, trg_skeleton, count, steps_per_epoch, style_labels, char_labels")
...
def _parse_function(example, a):
    ...
    with tf.name_scope("target_font"):
        trg_font = transform(a_images)

    with tf.name_scope("target_skeleton"):
        trg_skeleton = transform(b_images)
    ...
    return trg_font, trg_skeleton, style_label, character_label, path
...
def load_examples(args):
   ...
   # batch contains the input images , labels and target images for the model
    trg_font, trg_skeleton, style_label, char_label, path = batch
